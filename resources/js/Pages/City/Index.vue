...
function submit() {
    if (editId.value) {
        form.patch(route("city.update", editId.value), {
            onSuccess: () => {
                closeForm();
            }
        })
...
function deleteCity() {
    form.delete(route("city.delete", editId.value), {
        onSuccess: () => {
    ...
    <AppLayout title="Dashboard">
        <template #header>
            <div class="flex items-center justify-center">
                <h2 class="font-semibold text-xl text-gray-800 mr-5 leading-tight">Список городов</h2>
                <div>
                    <PrimaryButton @click="(showModal = true)" v-if="cities.length > 0">Добавить город</PrimaryButton>
                </div>
            </div>
        </template>

        <div class="py-12" v-if="cities.length > 0">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-10">
                    <div v-for="city in cities" :key="city.id"
                        class="mb-5 border-4 p-10 flex items-center justify-between border-black">
                        <div class="text-2xl">
                            <h1 class="font-bold">Город - {{ city.name }}</h1>
                            <h1 class="font-bold">Путевых листов - {{ city.sheets.length }}</h1>
                        </div>
                        <div>
                            <PrimaryButton class="mr-2" @click="edit(city)">Редактировать</PrimaryButton>
                            <DangerButton @click="showDeleteForm(city.id)">Удалить</DangerButton>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div v-else class="flex items-center justify-center mt-10">
            <PrimaryButton @click="(showModal = true)">Добавить город</PrimaryButton>
        </div>
