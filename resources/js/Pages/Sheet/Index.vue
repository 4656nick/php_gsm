...
function submit() {
    if (editId.value) {
        form.patch(route("sheet.update", editId.value), {
            onSuccess: () => {
                closeForm();
            }
        })
    } else {
        form.post(route("sheet.create"), {
            onSuccess: () => {
                closeForm();
            }
        })
...
function deleteSheet() {
    form.delete(route("sheet.delete", editId.value), {
        onSuccess: () => {
    ...
    <AppLayout title="Dashboard">
        <template #header>
            <div class="flex items-center justify-center">
                <h2 class="font-semibold text-xl text-gray-800 mr-5 leading-tight">Путевые листы</h2>
                <div>
                    <PrimaryButton @click="(showModal = true)" v-if="sheets.length > 0">Добавить путевой лист
                    </PrimaryButton>
                </div>
            </div>
        </template>

        <div class="py-12" v-if="sheets.length > 0">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-10">
                    <div v-for="sheet in sheets" :key="sheet.id"
                        ...
                        <div class="text-2xl">
                            <h1 class="font-bold">Название путевого листа - {{ sheet.name || 'Неизвестно' }} </h1>
                            <h1 class="font-bold">Город - {{ sheet.city ? sheet.city.name : 'Неизвестно' }}</h1>
                        </div>
                        <div>
                            <PrimaryButton class="mr-2" @click="edit(sheet)">Редактировать</PrimaryButton>
                            <DangerButton @click="showDeleteForm(sheet.id)">Удалить</DangerButton>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div v-else class="flex items-center justify-center mt-10">
            <PrimaryButton @click="(showModal = true)">Добавить путевой лист</PrimaryButton>
        </div>
